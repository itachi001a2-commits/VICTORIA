...
import { formatMoney, logAudit } from "@/store";
...
export default function TicketsScreen({ user }: TicketsScreenProps) {
  ...
  const annulTicket = async (ticket: any) => {
    ...
    const netDeduction = Number(ticket.netDeduction || ticket.total) || 0;
    const commissionAmount = Number(ticket.total) * (Number(ticket.commission || 0) / 100);

    try {
      // Recover credit to seller
      ...
      logAudit({ type: "ticket_annul", userId: ticket.userId, userName: ticket.seller || "", groupId: user.groupId, amount: Number(ticket.total), description: `Ticket anulado: ${ticket.code} | Credito recuperado: $${formatMoney(netDeduction)}`, ticketCode: ticket.code });
